...
if [ "$1" = 'consul' ]; then
    ...
    if [ "$(stat -c %u /consul/data)" != "$(id -u burbank)" ]; then
        chown -R burbank:burbank /consul/data
    fi
    if [ "$(stat -c %u /consul/config)" != "$(id -u burbank)" ]; then
        chown -R burbank:burbank /consul/config
    fi
    ...
    if [ ! -z ${CONSUL_ALLOW_PRIVILEGED_PORTS+x} ]; then
        setcap "cap_net_bind_service=+ep" $(readlink -f $(which consul))
    fi
    if [ "$(id -u)" = '0' ]; then
      set -- su-exec burbank:burbank "$@"
    fi
fi
