...
if [ "$1" = 'vault' ]; then
    ...
    if [ "$(stat -c %u /vault/config)" != "$(id -u burbank)" ]; then
        chown -R burbank:burbank /vault/config || echo "Could not chown /vault/config (may not have appropriate permissions)"
    fi

    # If the logs dir is bind mounted then chown it
    if [ "$(stat -c %u /vault/logs)" != "$(id -u burbank)" ]; then
        chown -R burbank:burbank /vault/logs
    fi

    # If the file dir is bind mounted then chown it
    if [ "$(stat -c %u /vault/file)" != "$(id -u burbank)" ]; then
        chown -R burbank:burbank /vault/file
    fi
